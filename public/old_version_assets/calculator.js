  ...
  static calculateRecentStats(summoners) {
    ...
    return Promise.all(promises)
      .then(function(data) {
        ...
        summoners.WRs.push(
          Math.round((rs.aggregateWins / rs.aggregateGames) * 100)
        );
      })
      .catch(function(error) {
        console.log(error);
  ...
  static calculateSeasonRates(summoners) {
    ...
    var promises = summoners.arr.map(function(summonerId) {
      var url =
        ss.baseUrl +
        summonerId +
        "/summary?season=SEASON2017&api_key=" +
        API_KEY;
      return request(url)
        .then(function(response) {
          ...
          ss.games =
            ss.wins + summonerStats.playerStatSummaries[ss.rankedIndex].losses;
          ss.aggregateGames += ss.games;
          if (ss.games == 0) {
            ss.summonerRates.set(summonerId, 0);
          } else {
            ss.summonerRates.set(
              summonerId,
              Math.round((ss.wins / ss.games) * 100)
            );
          }
        })
        .catch(function(error) {
    ...
    return Promise.all(promises)
      .then(function(data) {
        ...
        if (ss.aggregateGames) {
          summoners.seasonWRs.push(
            Math.round((ss.aggregateWins / ss.aggregateGames) * 100)
          );
        } else {
...
function countGames(response, rs, summonerId) {
  ...
  rs.summonerRates.set(
    summonerId,
    Math.round((rs.wins / rs.rankedGames) * 100)
  );
